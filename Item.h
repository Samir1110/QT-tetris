...
typedef	QList<QPoint> PointList;//自定义类型PointList，由多个QPoint点组成的列表，用于表示游戏元素在4x4的矩阵中的位置
...
public:
    ...
    void Load(ItemType type, ItemDirection direction, QString text);
    void InitPoints(ItemType type, ItemDirection direction);	//根据元素类型、方向，初始化4个坐标点
    void InitPoints1(ItemType type, ItemDirection direction, QString text);
